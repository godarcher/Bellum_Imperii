...
# Start troops in formation
	...
	(.1, 0, 0, [
		...
		(try_begin),
			...
			(party_set_slot, 2, slot_party_gk_order_hold_over_there, 3),
		(else_try),
...
#implement HOLD OVER THERE when player lets go of key/etc
	(.5, 0, 0, [(neg|main_hero_fallen)], [
		...
		(try_begin),
			(party_slot_ge, 2, slot_party_gk_order_hold_over_there, 2),
			...
			(assign, ":radius_enemy_battlegroup", 0),
			(try_begin),
				(party_slot_eq, 2, slot_party_gk_order_hold_over_there, 2), #Action key wasn't down (it would eq 3 then)
				(try_for_range, ":team", 0, 4),
	...
	(ti_on_agent_spawn, 0, 0, [], [(store_trigger_param_1, ":agent"),(call_script, "script_prebattle_agent_fix_division", ":agent")]), ##FOR PBOD add prebattle_, remove without PBOD
]
...
field_ai_triggers = [
  ...
  (0, 0, 2, [(key_clicked, "$key_special_0"), (get_player_agent_no, "$fplayer_agent_no"),(agent_is_alive,"$fplayer_agent_no"), #spearwall_trigger_2
...
AI_triggers = [  
	...
	(0.1, 0, 0, [(party_slot_eq, "p_main_party", slot_party_pref_formations, 1)], [	
		...
		(else_try), #Caba addition
			(eq, "$battle_phase", BP_Jockey),
			(store_mission_timer_a, reg0),
			(gt, reg0, AI_force_action_time),
			(assign, "$battle_phase", BP_Fight),
		(try_end),
	...
	(1, 0, ti_once, [ ##caba-encorporate in another trigger?? standard death-cam ones?
		...
		(try_for_agents, ":agent"),	#reassign agents to the divisions AI uses
			(agent_is_alive, ":agent"),
			(call_script, "script_prebattle_agent_fix_division", ":agent"), ##FOR PBOD add prebattle_, remove without PBOD
		(try_end),
...
common_battle_get_riders = (
  0, 0, ti_once, [],
  [
		(try_for_agents, ":agent"),
			(agent_is_human, ":agent"),
			(agent_get_horse, ":horse", ":agent"),
			(ge, ":horse", 1), #eq is mounted
			(agent_set_slot, ":agent", slot_agent_is_rider, 1),
		(try_end),
		(get_player_agent_no, ":player"),
		(agent_get_horse, ":horse2", ":player"),
		(ge, ":horse2", 1),
		(assign, "$player_horse", ":horse2"),
		(assign, "$g_player_has_horse", 1),
    ])

common_battle_kill_rider_on_fall = (
  5, 0, 0, [],
	[	
		(try_for_agents,":agent"),
			(agent_is_alive,":agent"),
			(agent_is_human,":agent"),
			(agent_slot_eq, ":agent", slot_agent_is_rider, 1),
			(agent_get_troop_id, ":troop_no", ":agent"),
			(neg|troop_is_hero, ":troop_no"),
			(get_player_agent_no, ":pagent"),
			(neq, ":agent", ":pagent"),
			(agent_get_horse, ":horse", ":agent"),
			(lt, ":horse", 1), #no horse
			(agent_slot_eq, ":agent", slot_agent_survived_fall, 0),
			(store_random_in_range, ":range", 1, 100),
			(try_begin),
				(le, ":range", 33),
				(agent_set_hit_points,":agent",0,1),
				(agent_deliver_damage_to_agent,":agent",":agent"),
				(str_store_troop_name, s60, ":agent"),
				(display_message, "@{s60} his horse died and fell on him, he did not survive"),
				(agent_set_slot, ":agent", slot_agent_survived_fall, 0),
			(else_try),
				(agent_set_slot, ":agent", slot_agent_survived_fall, 1),
		    (try_end),
		(try_end),
	]) 

common_horse_rear_custom = (
   2, 0, 0, [(key_is_down, key_j)],
    [
		(get_player_agent_no, ":player_agent"),
		(agent_get_horse, ":horse", ":player_agent"),
        (ge, ":horse", 0), #asure mounted
		...
		(try_for_agents, ":agent"),
			...
			(neq, ":agent_team", ":player_team"), #player enemy
			(agent_get_position, pos2, ":agent"),
		    (neg|position_is_behind_position, pos2, pos1), #pos2 behind pos1 (enemy in front of horse)
			(get_distance_between_positions, ":dist", pos1, pos2),
			(le, ":dist", 250),
			(agent_get_item_id, ":horse_item", ":horse"),
			(item_get_horse_speed, ":horse_speed", ":horse_item"),
			(item_get_horse_charge_damage, ":horse_charge_damage", ":horse_item"),
		    (val_add, ":horse_charge_damage", ":horse_speed"),
			(agent_deliver_damage_to_agent, ":agent", ":horse", ":horse_charge_damage"),
			(agent_set_animation, ":agent", "anim_shield_strike"),
			(assign, reg1, ":horse_charge_damage"),
			(str_store_troop_name, s1, ":agent"),
			(display_message,"@Your horse hit {s1} for {reg1} damage!",0x6495ed),		
		(try_end),	
	 ])

common_horse_rear_high = (
   2, 0, 0, [],
    [
		(try_for_agents, ":agent"),
			(agent_is_human, ":agent"),
			(agent_is_alive, ":agent"),
			(agent_get_horse, ":horse", ":agent"),
            (ge, ":horse", 1), #asure mounted
			(agent_get_position, pos1, ":horse"),
			(position_move_y, pos1, 200),
			(position_set_z_to_ground_level, pos1),
			(agent_get_position, pos2, ":horse"),
			(get_distance_between_positions, ":dist", pos1, pos2),
			(gt, ":dist", 600), #meaning in 2 meter goes six meter down
			(agent_set_animation, ":horse", "anim_horse_rear"),
		(try_end),
	 ])

common_battle_call_horse = (
   0,0,0, [(key_clicked, key_t),(neg|main_hero_fallen),(eq, "$g_player_has_horse", 1)],
    [
		(play_sound,"snd_horse_low_whinny"),
        (get_player_agent_no, ":player_agent"),
        (agent_get_position, pos1, ":player_agent"),
		(agent_get_horse, ":horse", ":player_agent"),
		(lt, ":horse", 1), #not using horse now
		(store_skill_level, ":riding_skill_level", "skl_riding", "trp_player"), 
		(ge, ":riding_skill_level", 5), #high riding skill :D
		(agent_get_position, pos2, ":player_agent"),
		(agent_is_alive, "$player_horse"),
		(agent_get_position, pos1, "$player_horse"), #we stored player horse in another mt
		(get_distance_between_positions,":distance",pos2,pos1),
		(ge,":distance",250), #distance big otherwise not going
		(agent_set_scripted_destination,"$player_horse",pos1,1),
...
common_battle_rage = (
  ...
  [
	...
	(else_try),
		(assign, "$rageused", 2),
		(set_show_messages, 1),
		(display_message, "@Adrenaline rushes trough your body, nearby death inspires you to heroics", 0xff0000),
		(set_show_messages, 0),
		(val_add, ":playerhp", 27),
  ...
  [
	...
	(try_begin),
		...
		(try_begin),
			...
			(this_or_next|eq, ":troop", "trp_roman_bearer"),
			(this_or_next|eq, ":troop", "trp_roman_prae_bearer"), 
			(this_or_next|eq, ":troop", "trp_roman_gaul_bearer"), 
			(eq, ":troop", "trp_roman_gaul_prae_bearer"), 
			(set_show_messages, 1),
			(display_message,"@One of our standards has been dropped, shame lies upon us all!",0xED7864),
			(set_show_messages, 0),
		(try_end),
	...
	(else_try),
		...
		(try_begin),
			...
			(eq, ":troop", "trp_roman_gaul_prae_bearer"), 
			(set_show_messages, 1),
			(display_message,"@An enemy standard has been dropped, they shall live in eternal shame!",0x95ED64),
			(set_show_messages, 0),
		(else_try),
			(eq, ":troop", "trp_horatius_cocles"),
			(set_show_messages, 1),
			(display_message,"@The great Horatius Cocles was slain!",0x95ED64),
			(set_show_messages, 0),
		(else_try),
			(eq, ":troop", "trp_roman_consul"),
			(set_show_messages, 1),
			(display_message,"@A roman consul, symbol of the roman empire, was slain!",0x95ED64),
			(set_show_messages, 0),
		(else_try),
			(eq, ":troop", "trp_kingdom_1_lord"),
			(set_show_messages, 1),
			(display_message,"@A sad day in the history of the roman empire, the emperor was slain in battle!",0xaf0b66),
			(set_show_messages, 0),
		(else_try),
			(eq, ":troop", "trp_knight_1_1"),
			(set_show_messages, 1),
			(display_message,"@A sad day in the history of the roman empire, general Flavius was slain in battle!",0x95ED64),
			(set_show_messages, 0),
		(try_end),
...
common_battle_decapitation = ( #head
    ...
    [
	  ...
	  (get_distance_between_positions, ":distance", pos1, pos2),
	  (is_between, ":distance", 0, 250), #no arrow or very less power hitting
	  (store_agent_hit_points, ":hp", ":agent", 1),
	  (store_random_in_range, ":range", 0, 100),
	  (le, ":range", 23),
	  (agent_get_troop_id, ":troop_no", ":agent"), #agent get troop id
	  (neg|troop_is_hero, ":troop_no"), #agent is no hero
	  (neq, ":troop_no", "trp_horatius_cocles"),
	  (neq, ":troop_no", "trp_kingdom_1_lord"),
	  (neq, ":troop_no", "trp_knight_1_1"),
	  (neq, ":troop_no", "trp_roman_consul"),
	  (store_random_in_range, ":secondrange", 0, 100), #7 options
	  #general needs end
	  (try_begin),
	    #HEAD
		(le, ":secondrange", 34),
	  ...
	  (else_try),
		...
		(try_begin),
			...
			(assign, ":new_hand_item", "itm_man_left_chopped_hand"), 
			(agent_equip_item, ":agent", ":new_hand_item"),
			#drop wielded weapons
			(try_begin),
				(agent_get_wielded_item,":weapon",":agent", 1), #left hand 
				(try_begin), 
					(eq, ":weapon", -1),
					(agent_get_wielded_item,":weapon",":agent", 0), #right hand 
					(agent_unequip_item, ":agent", ":weapon", 1), #two handed
				...
				(else_try),
					(neq, ":weapon", 0),
					(agent_unequip_item, ":agent", ":weapon", 2), #left hand
					(copy_position, pos2, pos4),
					(position_set_z_to_ground_level, pos4),
					(set_spawn_position, pos4),
					(spawn_item, ":weapon"),
      ...
      (else_try), 
			...
			(try_begin),
				...
				(assign, ":hand_id", reg0),
				(prop_instance_enable_physics, ":hand_id", 1),
				(assign, ":new_hand_item", "itm_man_right_chopped_hand"), 
				(agent_equip_item, ":agent", ":new_hand_item"),
				#drop wielded weapons
				(try_begin),
					(agent_get_wielded_item,":weapon",":agent", 0), #right hand 
					(neq, ":weapon", 0),
					(agent_unequip_item, ":agent", ":weapon", 1), #right hand
					(copy_position, pos2, pos4),
					(position_set_z_to_ground_level, pos4),
					(set_spawn_position, pos4),
					(spawn_item, ":weapon"),
	  ...
	  (else_try),
			...
			(gt, ":secondrange", 56), 
			(le, ":secondrange", 67),
			(ge, ":damage", 35), #stronger blow then just one hand
			(try_begin),
				(play_sound_at_position, "snd_decapitation", pos0),
				(particle_system_burst, "psys_decap_blood", pos2, 5),
				(particle_system_burst, "psys_decap_blood", pos2, 5),
		...
		(else_try),
			...
			(gt, ":secondrange", 67), 
			(le, ":secondrange", 78),
			(ge, ":damage", 35), #strongest blow of all because legs are harder to decap
			(play_sound_at_position, "snd_decapitation", pos0),
			(set_spawn_position, pos0),
			(particle_system_burst, "psys_decap_blood", pos2, 5),
			(particle_system_burst, "psys_decap_blood", pos2, 5),
			...
			(agent_equip_item, ":agent", "itm_invisible_leg_l"),
			(store_agent_hit_points, ":hp", ":agent", 1),
			#bleed
			(agent_set_animation, ":agent", "anim_stand_to_crouch", 1),
			#scream
			(agent_play_sound, ":agent", "snd_man_hit"),
			(val_add, ":damage", 10),
			(set_trigger_result, ":damage"),
		(else_try), 
			...
			(assign, ":leg_id", reg0),
			(prop_instance_enable_physics, ":leg_id", 1),
			(agent_equip_item, ":agent", "itm_invisible_leg_r"),
			(store_agent_hit_points, ":hp", ":agent", 1),
			#bleed
			(agent_set_animation, ":agent", "anim_stand_to_crouch", 1),
			#scream
		...
		(else_try), 
			...
			(gt, ":secondrange", 89), 
			(le, ":secondrange", 100),
			(ge, ":damage", 38), #strongest blow of all because legs are harder to decap
			(play_sound_at_position, "snd_decapitation", pos0),
			(set_spawn_position, pos0),
			(particle_system_burst, "psys_decap_blood", pos2, 5),
			(particle_system_burst, "psys_decap_blood", pos2, 5),
			...
			(agent_equip_item, ":agent", "itm_invisible_legs"),
...
common_battle_notifications = (
  20, 0, 0, [],
	...
	#(team_give_order, 2, grc_everyone, mordr_stand_closer),
	#(team_give_order, 2, grc_everyone, mordr_stand_closer),
	#(start_presentation, "prsnt_troop_ratio_bar"),
	(set_show_messages, 1),
...
#-----------------------------------------------------------------
  ...
  (
   ...
   [
     ...
     (try_begin), 
       (this_or_next|eq, ":dead_agent_troop_no", "trp_roman_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_burgundiones_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_dacian_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_parthian_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_maurian_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_roman_prison_guard_gallia"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_roman_prison_guard_illyricum"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_roman_prison_guard_asia"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_roman_prison_guard_hispania"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_celtic_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_marcomanni_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_frisii_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_sarmatian_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_chatti_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_langobardi_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_nabatean_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_alanian_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_bosporan_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_aesti_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_garamantian_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_suebi_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_jazygian_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_bastarnian_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_armenian_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_rugii_prison_guard"),
       (eq, ":dead_agent_troop_no", "trp_cimbri_prison_guard"),
...
##          (agent_get_team, reg3, ":dead_agent_no"),          
    ...
    ] + order_volley_triggers + dplmc_battle_mode_triggers + caba_order_triggers + field_ai_triggers,
  ),
...
#         (tutorial_box, "str_trainer_help_2", "@Tutorial"),
  ...
  (
    ...
    [
      (16, mtef_visitor_source|mtef_team_0,af_override_everything,aif_start_alarmed,1,[itm_thraex_shield,itm_roman_cav_sword_1,itm_practice_boots]),
      ...
      (19, mtef_visitor_source|mtef_team_3,af_override_everything,aif_start_alarmed,1,[itm_roman_cav_sword_1, itm_practice_boots]),
...
#      (ti_before_mission_start, 0, 0, [], [(set_rain, 1,100)]),
  ...
  (
    ...
    [
      (0,mtef_visitor_source|mtef_team_0,af_override_all,aif_start_alarmed,1,[itm_practice_bow,itm_practice_arrows,itm_practice_horse,itm_ad_roman_tunic_pectoral_01, itm_murmillo_helmet]),
      (1,mtef_visitor_source|mtef_team_0,af_override_all,aif_start_alarmed,1,[itm_roman_cav_sword_1, itm_ad_roman_tunic_pectoral_01]),
      (2,mtef_visitor_source|mtef_team_0,af_override_all,aif_start_alarmed,1,[itm_roman_cav_sword_1,itm_practice_horse,itm_ad_roman_tunic_pectoral_01, itm_murmillo_helmet]),
      (3,mtef_visitor_source|mtef_team_0,af_override_all,aif_start_alarmed,1,[itm_roman_spear_2,itm_thraex_shield,itm_practice_horse,itm_ad_roman_tunic_pectoral_01, itm_murmillo_helmet]),
      (4,mtef_visitor_source|mtef_team_0,af_override_all,aif_start_alarmed,1,[itm_practice_bow,itm_practice_arrows, itm_practice_dagger, itm_ad_roman_tunic_pectoral_01]),
      (5,mtef_visitor_source|mtef_team_0,af_override_all,aif_start_alarmed,1,[itm_roman_gladius_1,itm_thraex_shield,itm_ad_roman_tunic_pectoral_01]),
      (6,mtef_visitor_source|mtef_team_0,af_override_all,aif_start_alarmed,1,[itm_roman_cav_sword_1,itm_practice_horse,itm_ad_roman_tunic_pectoral_01]),
      (7,mtef_visitor_source|mtef_team_0,af_override_all,aif_start_alarmed,1,[itm_roman_spear_2,itm_thraex_shield,itm_practice_horse,itm_ad_roman_tunic_pectoral_01, itm_murmillo_helmet]),

      (8,mtef_visitor_source|mtef_team_1,af_override_all,aif_start_alarmed,1,[itm_practice_bow,itm_practice_arrows,itm_practice_dagger, itm_ad_roman_tunic_pectoral_01]),
      (9,mtef_visitor_source|mtef_team_1,af_override_all,aif_start_alarmed,1,[itm_roman_spear_2,itm_thraex_shield,itm_practice_horse,itm_ad_roman_tunic_pectoral_01,itm_murmillo_helmet]),
      (10,mtef_visitor_source|mtef_team_1,af_override_all,aif_start_alarmed,1,[itm_roman_cav_sword_1,itm_ad_roman_tunic_pectoral_01]),
      (11,mtef_visitor_source|mtef_team_1,af_override_all,aif_start_alarmed,1,[itm_roman_gladius_1,itm_thraex_shield,itm_ad_roman_tunic_pectoral_01, itm_murmillo_helmet]),
      (12,mtef_visitor_source|mtef_team_1,af_override_all,aif_start_alarmed,1,[itm_practice_bow,itm_practice_arrows,itm_practice_horse,itm_ad_roman_tunic_pectoral_01]),
      (13,mtef_visitor_source|mtef_team_1,af_override_all,aif_start_alarmed,1,[itm_roman_spear_2,itm_thraex_shield,itm_practice_horse,itm_ad_roman_tunic_pectoral_01,itm_murmillo_helmet]),
      (14,mtef_visitor_source|mtef_team_1,af_override_all,aif_start_alarmed,1,[itm_roman_cav_sword_1,itm_ad_roman_tunic_pectoral_01]),
      (15,mtef_visitor_source|mtef_team_1,af_override_all,aif_start_alarmed,1,[itm_roman_gladius_1,itm_thraex_shield,itm_ad_roman_tunic_pectoral_01]),

      (16,mtef_visitor_source|mtef_team_2,af_override_all,aif_start_alarmed,1,[itm_practice_bow,itm_practice_arrows,itm_practice_horse,itm_ad_roman_tunic_pectoral_01, itm_murmillo_helmet]),
      (17,mtef_visitor_source|mtef_team_2,af_override_all,aif_start_alarmed,1,[itm_roman_cav_sword_1,itm_ad_roman_tunic_pectoral_01, itm_murmillo_helmet]),
      (18,mtef_visitor_source|mtef_team_2,af_override_all,aif_start_alarmed,1,[itm_roman_cav_sword_1,itm_practice_horse,itm_ad_roman_tunic_pectoral_01, itm_murmillo_helmet]),
      (19,mtef_visitor_source|mtef_team_2,af_override_all,aif_start_alarmed,1,[itm_roman_spear_2,itm_thraex_shield,itm_practice_horse,itm_ad_roman_tunic_pectoral_01, itm_murmillo_helmet]),
      (20,mtef_visitor_source|mtef_team_2,af_override_all,aif_start_alarmed,1,[itm_practice_bow,itm_practice_arrows,itm_practice_dagger, itm_ad_roman_tunic_pectoral_01, itm_murmillo_helmet]),
      (21,mtef_visitor_source|mtef_team_2,af_override_all,aif_start_alarmed,1,[itm_roman_gladius_1,itm_thraex_shield,itm_ad_roman_tunic_pectoral_01]),
      (22,mtef_visitor_source|mtef_team_2,af_override_all,aif_start_alarmed,1,[itm_roman_cav_sword_1,itm_practice_horse,itm_ad_roman_tunic_pectoral_01]),
      (23,mtef_visitor_source|mtef_team_2,af_override_all,aif_start_alarmed,1,[itm_roman_spear_2,itm_thraex_shield,itm_practice_horse,itm_ad_roman_tunic_pectoral_01, itm_murmillo_helmet]),

      (24,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_practice_bow,itm_practice_arrows,itm_practice_horse,itm_ad_roman_tunic_pectoral_01, itm_murmillo_helmet]),
      (25,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_roman_cav_sword_1,itm_ad_roman_tunic_pectoral_01, itm_murmillo_helmet]),
      (26,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_roman_cav_sword_1,itm_practice_horse,itm_ad_roman_tunic_pectoral_01, itm_murmillo_helmet]),
      (27,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_roman_spear_2,itm_thraex_shield,itm_practice_horse,itm_ad_roman_tunic_pectoral_01, itm_murmillo_helmet]),
      (28,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_practice_bow,itm_practice_arrows,itm_practice_dagger, itm_ad_roman_tunic_pectoral_01, itm_murmillo_helmet]),
      (29,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_roman_gladius_1,itm_thraex_shield,itm_ad_roman_tunic_pectoral_01]),
      (30,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_roman_cav_sword_1,itm_practice_horse,itm_ad_roman_tunic_pectoral_01]),
      (31,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_roman_spear_2,itm_thraex_shield,itm_practice_horse,itm_ad_roman_tunic_pectoral_01, itm_murmillo_helmet]),
#32
      (32, mtef_visitor_source|mtef_team_1,af_override_all,aif_start_alarmed,1,[itm_roman_cav_sword_1]),
      (33,mtef_visitor_source|mtef_team_2,af_override_all,aif_start_alarmed,1,[itm_practice_staff]),
      (34,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_roman_gladius_1, itm_thraex_shield]),
      ...
      (37,mtef_visitor_source|mtef_team_2,af_override_all,aif_start_alarmed,1,[itm_roman_gladius_1, itm_thraex_shield]),
      (38,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_roman_cav_sword_1]),
      (39,mtef_visitor_source|mtef_team_4,af_override_all,aif_start_alarmed,1,[itm_practice_staff]),
#40-49 not used yet
      (24,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_practice_bow,itm_practice_arrows,itm_practice_horse,itm_ad_roman_tunic_pectoral_01, itm_murmillo_helmet]),
      (24,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_roman_cav_sword_1,itm_ad_roman_tunic_pectoral_01, itm_murmillo_helmet]),
      (24,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_roman_cav_sword_1,itm_practice_horse,itm_ad_roman_tunic_pectoral_01, itm_murmillo_helmet]),
      (24,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_roman_spear_2,itm_thraex_shield,itm_practice_horse,itm_ad_roman_tunic_pectoral_01, itm_murmillo_helmet]),
      (24,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_practice_bow,itm_practice_arrows,itm_practice_dagger, itm_ad_roman_tunic_pectoral_01, itm_murmillo_helmet]),
      (24,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_thraex_shield,itm_ad_roman_tunic_pectoral_01]),
      (24,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_roman_cav_sword_1,itm_practice_horse,itm_ad_roman_tunic_pectoral_01]),
      (24,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_roman_spear_2,itm_thraex_shield,itm_practice_horse,itm_ad_roman_tunic_pectoral_01, itm_murmillo_helmet]),
      (24,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_practice_bow,itm_practice_arrows,itm_practice_horse,itm_ad_roman_tunic_pectoral_01, itm_murmillo_helmet]),
      (24,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_practice_bow,itm_practice_arrows,itm_practice_horse,itm_ad_roman_tunic_pectoral_01, itm_murmillo_helmet]),
...
#used for torunament master scene
  ...
  (
    ...
    "You enter a melee fight in the arena.",
    [    
	  (0, mtef_visitor_source|mtef_team_0,af_override_all,aif_start_alarmed,1,[itm_ad_roman_tunic_pectoral_01]),
	  (16, mtef_visitor_source|mtef_team_1,af_override_all,aif_start_alarmed,1,[itm_ad_roman_tunic_pectoral_01]),
    ],
    [
      common_inventory_not_available,
      (ti_tab_pressed, 0, 0, [(display_message, "str_cannot_leave_now")], []),
...
##  ),
  ...
  (
    ...
    [
        ...
        (3,mtef_visitor_source,af_override_everything,0,1,[itm_courtly_outfit, itm_ankle_boots]),
    ...
    [
      ...
      (0, 0, 0, [(try_begin),
                 ...
                 (else_try),
                   ...
                   (reset_visitors),
                   (set_visitor,5,"trp_roman_n_legionary"),
                   (set_visitor,6,"trp_roman_n_legionary"),
                   (set_visitor,7,"trp_roman_n_legionary"),
