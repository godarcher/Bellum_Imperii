...
def generate_presentation_load_script(_mod_options = mod_options):
	...
	opblock.Append([                       
		...
		(create_text_overlay, overlay_var, "@%s" % mod_name + "^Mod Options", tf_center_justify|tf_with_outline),
		...
		(create_text_overlay, overlay_var, s1, tf_center_justify|tf_with_outline),
		(overlay_set_color, overlay_var, 0xFFFFFFFF),
		(position_set_x, pos1, 800),
...
def generate_presentation_event_state_change_script(_mod_options = mod_options):
	...
	opblock.Append([                       
		...
		(else_try),
			...
			(eq, ":object", ":overlay"),
			(call_script, "script_prebattle_set_default_prefs", 0),	
			(try_begin),		  
				(eq, "$g_is_quick_battle", 1),
				(presentation_set_duration, 0),
			# (else_try),
